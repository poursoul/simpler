...
__attribute__((visibility("default"), weak)) PTO_DEVICE_FUNC void aicpu_orchestration_entry(
    ...
    if (mode == 1) {
        ...
        rt_submit_aic_task(FUNC_FILL_ALLOC_AIC, fill_full_args);
    ...
    if (mode == 7) {
        ...
        const Tensor &after_add = fill_args.tensor(0).ref();
        write_descriptor_checks(after_add, check_output, 16, n, output);
        dcci(check_output, SINGLE_CACHE_LINE, CACHELINE_OUT);
