...
extern "C" PTO_DEVICE_FUNC int32_t pto_call_linked_kernel_aiv(int32_t func_id, __gm__ int64_t *args)
    __attribute__((weak));
#endif
#if defined(__CPU_SIM) && !DIST_ONBOARD_AICORE
extern "C" void framework_bind_runtime(PTO2Runtime *rt);
#endif

#if DIST_AICORE_TARGET
...
// Submit payloads are keyed by each core's replay-local task id. They need a
// deeper retention window than the tiny private execution ring, but stay
// per-core so submit-side descriptor reads do not become cross-core GM traffic.
...
struct DistTensorMap {
    ...
    // that reference host-uploaded Tensors, payload entries the engine
...
struct DistCoreTraceState {
    ...
    // span abuts this one with zero gap.
    uint64_t trace_last_ns;
    uint64_t trace_last_cpu;

    // Per-core static dependency edges and slot-release edges (tracing only).
    std::vector<DepEdge> dep_edges;
    std::vector<DepEdge> slot_edges;
};

DistCoreTraceState g_trace_cores[RUNTIME_MAX_WORKER];
#endif
...
struct DistCore {
    ...
    // __gm__-empty expansion collapses this to a plain reference.
...
struct DistTaskCell {
    ...
    volatile int64_t claimed;
    volatile uint64_t vend;
    uint8_t pad[kCacheLine - 2 * sizeof(int64_t) - sizeof(uint64_t)];
};
...
inline void trace_reset_core(DistCore *self) {
    ...
    ts.trace_last_cpu = 0;
    ts.trace.clear();
    // Pre-size the trace vector only when tracing is on (see g_trace_on), so
    // push_back never reallocs mid-run. Costs nothing on a normal run.
...
// Per-task claim: the task cell owns a cacheline, so this CAS is isolated from
// unrelated task flags. Cursor shards remain a diagnostic/progress mirror only;
// they cannot be the ownership primitive because cube/vector/alloc tasks form
// different sparse subsequences of the global task id stream.
PTO_DEVICE_FUNC bool claim(__gm__ volatile int64_t &cursor, int32_t N) {
    if (N < 0 || N >= kFlagCap) return false;
    __gm__ DistTaskCell &cell = task_cell(N);
#if DIST_ONBOARD_AICORE
    __gm__ int64_t *claimed = const_cast<__gm__ int64_t *>(&cell.claimed);
    const int64_t old = atomicMax(claimed, static_cast<int64_t>(1));
    if (old != 0) return false;
#else
    int64_t exp = 0;
    if (!atom_cas_strong(cell.claimed, exp, static_cast<int64_t>(1), __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) return false;
#endif
#if DIST_ONBOARD_AICORE
    (void)cursor;
#else
    atom_store(cursor, static_cast<int64_t>(N), __ATOMIC_RELEASE);
#endif
    return true;
}
...
PTO_DEVICE_FUNC void reset_task_cell(int32_t task_id) {
    ...
    atom_store(cell.claimed, 0, __ATOMIC_RELAXED);
...
// input arrays: the winner branch of dist_submit_impl passes task-payload
// tensors / scalars plus local fanin, and drain_block_won passes b.tensors /
// b.scalars / b.fanin reached through a __gm__ BuiltSubtask. Templating the
// three source-array pointer types lets both call sites bind naturally without
// a second overload — CCEC deduces the address-space qualifier on each pointer
// at instantiation time; sim just sees `const T *`.
...
PTO_DEVICE_FUNC bool has_free_won_slot(int32_t block) {
    __gm__ BlockWon &bw = g_dist.blocks[block];
    for (int32_t i = 0; i < kPrivateSlots; i++) {
        if (atom_load(bw.slots[i].state, __ATOMIC_ACQUIRE) == 0) return true;
    }
    return false;
}
...
PTO_DEVICE_FUNC inline __gm__ const Tensor &payload_tensor(const DistSubmitCtx &ctx, int32_t i) {
    return ctx.payload->tensors[i];
}
#else
PTO_DEVICE_FUNC inline TensorArgType payload_tag(const DistSubmitCtx &ctx, int32_t i) { return ctx.payload->tags[i]; }

PTO_DEVICE_FUNC inline const Tensor &payload_tensor(const DistSubmitCtx &ctx, int32_t i) {
    return ctx.payload->tensors[i];
}
#endif
...
PTO_DEVICE_FUNC TaskOutputTensors dist_submit_impl(PTO2Runtime *, const MixedKernels &mixed, const L0TaskArgs &args) {
    ...
    DistSubmitCtx ctx;
    dist_submit_begin(self, args, ctx);
    ...
    // then producer-map retirement. Output registration intentionally stays below
    // fan-in collection so INOUT does not self-match.
    if (!dist_submit_materialize_and_prepare_map(self, args, ctx, DistSubmitKind::Kernel)) return ctx.result;
    ...
    const bool won_slot_available = pc <= 1 || has_free_won_slot(self->block_id);
    if (type_match && won_slot_available) {
    ...
    int32_t fc = 0;
    if (is_winner) {
        fc = dist_submit_collect_fanin(ctx, fanin);
    }
    ...
    // (e) Winner only: the task payload already holds the stable argument
    // tensors/scalars for every slot. RingSlot and block.won deposits copy from
    // that payload directly instead of rebuilding a second stack-local tensor
    // array.
    auto scalars = ctx.payload->scalars;
    const int32_t sc = ctx.payload->scalar_count;
    ...
    if (g_trace_on && self->occupied_count >= kPrivateSlots - kWonReserve) {
        for (int32_t i = 0; i < kPrivateSlots; i++) {
            ...
            if (rs.occupied && rs.built) trace_state(self).slot_edges.push_back({N, rs.task_id});
        }
    }
#endif
    ...
    if (g_dist.heap_base != nullptr) {
        ...
        uint64_t wd_heap = 0;
        while (!fatal_set()) {
            const int32_t f = static_cast<int32_t>(atom_load(g_dist.frontier, __ATOMIC_ACQUIRE));
    ...
    build_ring_slot(
        self->slots[si], N, own_func_id, resolve_kernel_addr(runtime, own_func_id), ctx.payload->tensors, tc, scalars,
        sc, fanin, fc, own_sub_block, is_multicore, won_block, won_slot
    );
    self->occupied_count++;
    self->owned_total++;
    ...
    if (g_trace_on) {
        for (int32_t k = 0; k < fc; k++)
            trace_state(self).dep_edges.push_back({N, fanin[k]});
    }
#endif
    TRACE_LAP(self, N, -1, TracePhase::Commit);
    return ctx.result;
}
#endif  // !DIST_AICORE_TARGET
#if DIST_ONBOARD_AICORE
}  // namespace
#endif
...
// rt->ops. Host/sim definitions below run the full replay/ring backend; CCEC
// shares submit materialization, producer-map, fan-in, and output registration
// stages, then uses the current direct AICore execution backend.
// -----------------------------------------------------------------------------
...
#else
        result = *reinterpret_cast<const uint8_t *>(addr);
#endif
    } else if (esz == 2) {
#if DIST_ONBOARD_AICORE
        result = *reinterpret_cast<__gm__ const uint16_t *>(addr);
#else
        result = *reinterpret_cast<const uint16_t *>(addr);
#endif
    } else if (esz == 4) {
#if DIST_ONBOARD_AICORE
        result = *reinterpret_cast<__gm__ const uint32_t *>(addr);
#else
        result = *reinterpret_cast<const uint32_t *>(addr);
#endif
    } else {
#if DIST_ONBOARD_AICORE
        result = *reinterpret_cast<__gm__ const uint64_t *>(addr);
#else
        result = *reinterpret_cast<const uint64_t *>(addr);
#endif
    }
    return result;
}
...
// NOLINTNEXTLINE(modernize-avoid-variadic-functions): PTO2RuntimeOps legacy ABI.
...
void dist_log_warn(const char *, const char *, ...) {}         // NOLINT(modernize-avoid-variadic-functions)
void dist_log_debug(const char *, const char *, ...) {}        // NOLINT(modernize-avoid-variadic-functions)
void dist_log_info_v(const char *, int, const char *, ...) {}  // NOLINT(modernize-avoid-variadic-functions)
...
PTO_DEVICE_FUNC void ccec_advance_frontier() {
#if DIST_ONBOARD_AICORE
...
PTO_DEVICE_FUNC void ccec_complete_task(DistSubmitCtx &ctx) {
    if (ctx.self == nullptr) return;
    store_task_vend(ctx.task_id, ctx.self->heap_next);
    ccec_publish_flag(ctx.task_id);
    ccec_advance_frontier();
}
...
PTO_DEVICE_FUNC bool ccec_has_free_won_slot(int32_t block) {
#if DIST_ONBOARD_AICORE
    __gm__ BlockWon &bw = g_dist.blocks[block];
    ccec_invalidate_region(&bw, sizeof(BlockWon));
    for (int32_t i = 0; i < kPrivateSlots; i++) {
        if (bw.slots[i].state == 0) return true;
    }
    return false;
#else
    return has_free_won_slot(block);
#endif
}
...
PTO_DEVICE_FUNC bool ccec_claim_kernel_submit(const MixedKernels &mixed, DistSubmitCtx &ctx) {
    ...
    if (pc >= 2) {
        ...
        const int32_t block = ctx.task_id % block_count;
        ctx.joint = true;
        ...
        if (!ccec_has_free_won_slot(block)) return false;
...
PTO_DEVICE_FUNC int32_t ccec_drain_ready_slots_direct(__gm__ DistCore *self);
...
PTO_DEVICE_FUNC void ccec_wait_direct_slot_capacity(__gm__ DistCore *self) {
    if (self == nullptr) return;
    while (self->occupied_count >= kPrivateSlots - kWonReserve) {
        ccec_drain_block_won_direct(self);
        if (ccec_drain_ready_slots_direct(self) == 0) SPIN_WAIT_HINT();
    }
}
...
PTO_DEVICE_FUNC int32_t ccec_alloc_won_slot_direct(__gm__ DistCore *self, int32_t block) {
    ...
    while (true) {
        ...
        ccec_drain_block_won_direct(self);
        ccec_drain_ready_slots_direct(self);
        SPIN_WAIT_HINT();
    }
#endif
}
...
PTO_DEVICE_FUNC bool ccec_build_winner_slot(DistSubmitCtx &ctx, __gm__ RingSlot *slot) {
    ...
    const uint64_t fn_addr =
        ...
        0;
#else
        resolve_kernel_addr(g_dist.runtime, ctx.kernel_id);
#endif
    build_ring_slot(
...
PTO_DEVICE_FUNC void ccec_complete_slot(__gm__ RingSlot &slot, __gm__ DistCore *self) {
    DistSubmitCtx ctx;
    ctx.self = self;
...
PTO_DEVICE_FUNC bool ccec_slot_fanin_ready(__gm__ const RingSlot &slot) {
...
PTO_DEVICE_FUNC void ccec_slot_store_barrier() {
#if DIST_ONBOARD_AICORE
...
PTO_DEVICE_FUNC int32_t ccec_drain_ready_slots_direct(__gm__ DistCore *self) {
    ...
    for (int32_t i = 0; i < kPrivateSlots; i++) {
        ...
        if (ccec_try_execute_slot_direct(slot, self)) freed++;
    }
    return freed;
}
...
PTO_DEVICE_FUNC bool ccec_has_pending_won_direct(__gm__ DistCore *self) {
    if (self == nullptr || self->lane == LANE_AIC || self->lane == LANE_NONE) return false;
    __gm__ BlockWon &bw = g_dist.blocks[self->block_id];
    ccec_invalidate_region(&bw, sizeof(BlockWon));
    if (bw.any_pub == 0) return false;
...
PTO_DEVICE_FUNC void ccec_drain_to_completion(__gm__ DistCore *self) {
    if (self == nullptr) return;
    ...
    while (true) {
        ccec_drain_block_won_direct(self);
        const int32_t freed = ccec_drain_ready_slots_direct(self);
        ...
        const bool pending = ccec_has_pending_won_direct(self);
...
dist_submit_impl(PTO2Runtime *, const MixedKernels &mixed, const L0TaskArgs &args) {
    ...
    ccec_drain_block_won_direct(ctx.self);
    ccec_drain_ready_slots_direct(ctx.self);
...
DIST_API_ATTR PTO_DEVICE_FUNC TaskOutputTensors dist_alloc_tensors(PTO2Runtime *, const L0TaskArgs &args) {
    ...
    if (!dist_submit_materialize_and_prepare_map(self, args, ctx, DistSubmitKind::Alloc)) return ctx.result;
...
#if DIST_CONTROL_PLANE
[[maybe_unused]] const PTO2RuntimeOps g_dist_ops = {
