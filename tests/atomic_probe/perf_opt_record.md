...
### 14.6 Alloc 24/96 候选人口夹心对照

为量化 Tournament 下恢复 96 个 Alloc 候选的真实代价，临时只改两点：

- Alloc 从每 task 24 个候选恢复为全部 96 个 worker；
- local 仍为 4 组，QK/PV 仍为 32，SF/UP 仍为 64，其余代码、
  负载、CANN 和设备不变。

按 24 候选、96 候选、恢复 24 候选的顺序，每组运行 10 个独立
B256 perf-clock 进程；30/30 轮全部语义 PASS：

| Alloc 候选 | mean | median | range |
| ---------- | ----: | ------: | ----- |
| 24（前） | 1,461.146 us | 1,458.332 us | 1,452.002～1,477.541 us |
| 96 | 1,503.231 us | 1,501.308 us | 1,493.809～1,514.861 us |
| 24（后） | 1,459.275 us | 1,456.333 us | 1,442.775～1,491.128 us |

前后两组 24 候选 mean/median 只相差约 `0.128%/0.137%`。以两组
24 的中心值作夹心基准，96 候选的：

- mean 增加 `43.021 us`，回退 `2.946%`；
- median 增加 `43.976 us`，回退 `3.018%`。

96 候选只增加 `18,432` 次 Alloc local CAS：逻辑 Claim 从 55,296
增至 73,728，root 仍是每 task 4 个竞争者。旧 FetchMax 合同下 96
相比 24 曾增加 `542.868 us`；Tournament 将该候选人口代价压小约
