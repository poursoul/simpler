...
struct CpuOps {
    ...
    static inline int64_t Load(volatile int64_t *address) {
        // 保留原子 add-zero 路径，让 16 个 pthread 在同一批热点地址上竞争。
...
int main(int argc, char **argv) {
    ...
    if (!pa_scheduler::host::ValidateOnlySchedulerOptions(options) ||
        !pa_scheduler::host::ValidateWinnerWorkloadOptions(workload_options)) {
        return EXIT_FAILURE;
    }
#if PA_BUILD_PERF_CLOCK
    ...
    for (uint32_t run = 1; run <= options.runs; ++run) {
        ...
        // 固定创建 16 个参与者：worker 0..7 扮演 AIC，8..15 扮演 AIV。
        // 每线程独占 Scalar-local 统计对象，只调度 host 已构建的任务。
        ...
        for (uint32_t worker_id = 0; worker_id < pa_scheduler::kWorkers; ++worker_id) {
            ...
            workers.emplace_back([
                ...
                pa_scheduler::RunOnlyScheduler<CpuOps>(state_pointer, worker_id, role, stats);
        ...
        // 基线的泳道 JSON。
        const pa_scheduler::host::Metrics metrics = pa_scheduler::host::ValidateOnlyScheduler(
            *state, run, host_us,
        ...
        std::printf(
            "[PERF-E2E] perf_boundary_reads_per_worker=2 "
        ...
        if (!options.swimlane_json.empty()) {
            ...
            if (!pa_scheduler::host::ExportOnlySchedulerRecords(
#if PTO_FDWIC_SHARED_MAP
                    ...
                    *trace_header, options.swimlane_json,
                    ...
                    real_compute
                        ...
                        : "none",
                    options.final_barrier_shape, options.trace_atomics, "cpu",
                    read_trace_records
#if PTO_FDWIC_SHARED_MAP
                    , read_submit_claim_records
#endif
