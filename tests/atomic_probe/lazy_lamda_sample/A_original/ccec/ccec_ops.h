...
// This private header is included after the CCEC/PTO headers and
...
#if defined(PA_LAZY_SAMPLE_SPLIT_FINISH)
    __aicore__ static inline pa_scheduler::LazySampleSplitRuntimeState &LazySampleSplitState() {
#if defined(PA_BUILD_AIC)
        return ::pa_scheduler_lazy_sample_callback_state_aic;
#elif defined(PA_BUILD_AIV)
        return ::pa_scheduler_lazy_sample_callback_state_aiv;
#endif
    }

    __aicore__ static inline bool FinishLazySampleCallback(
        const pa_scheduler::LazySampleCallbackTicket *ticket, const pa_scheduler::TaskArgs *args
    ) {
#if defined(PA_BUILD_AIC)
        return ::pa_scheduler_lazy_sample_callback_finish_aic(ticket, args) != 0;
#elif defined(PA_BUILD_AIV)
        return ::pa_scheduler_lazy_sample_callback_finish_aiv(ticket, args) != 0;
#endif
    }
#endif
