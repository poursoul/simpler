...
if [[ "$TENSORMAP_MODE" == "private" ]]; then
    ...
    # S4.16a 将 shared Vector sidecar 物理容量扩到16但 active 保持8。
    # 定向测试锁定 SF/UP 的 task%8 地址、错误 role 零 atomic、
    # Cube/Alloc 不变，以及 b256 的32,768次 Vector Claim、active
    # 每线4,096次、后八条物理线零访问和每 task 唯一 winner。
