...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    ...
    __gm__ Tensor *temp_tensor = reinterpret_cast<__gm__ Tensor *>(args[1]);
    ...
    __gm__ float *temp = reinterpret_cast<__gm__ float *>(temp_tensor->buffer.addr) + temp_tensor->start_offset;
    for (uint64_t i = 0; i < n; i++) {
        temp[i] = input[i] + 1.0f;
    }
    for (uint64_t off = 0; off < n * sizeof(float); off += 64) {
        dcci(reinterpret_cast<__gm__ uint8_t *>(temp) + off, SINGLE_CACHE_LINE, CACHELINE_OUT);
    }
}
