...
if [[ "$TENSORMAP_MODE" == "private" ]]; then
    ...
    # S4.16b 在 S4.16a 的同址 capacity16 布局中启用全部十六条线。
    # 定向测试锁定 SF/UP 的 task%16 地址、错误 role 零 atomic、
    # Cube/Alloc 不变，以及 b256 的32,768次 Vector Claim、
    # 每线2,048次和每 task 唯一 winner。
