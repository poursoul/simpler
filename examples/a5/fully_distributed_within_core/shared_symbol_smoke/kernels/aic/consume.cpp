...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    ...
    __gm__ Tensor *produced_tensor = reinterpret_cast<__gm__ Tensor *>(args[1]);
    ...
    __gm__ float *produced =
        reinterpret_cast<__gm__ float *>(produced_tensor->buffer.addr) + produced_tensor->start_offset;
    __gm__ float *output = reinterpret_cast<__gm__ float *>(output_tensor->buffer.addr) + output_tensor->start_offset;
    for (uint64_t i = 0; i < n; i++) {
        output[i] = input[i] + produced[i] + 17.0f;
    }
