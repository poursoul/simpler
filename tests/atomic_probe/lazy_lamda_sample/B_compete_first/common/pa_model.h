...
// The original artifact is compiled without this selector.  The two selected
// artifacts deliberately share one compete-first/split-finish skeleton; only
// the builder's loser replay policy differs.
static_assert(
    PA_LAZY_SAMPLE_SHAPE_ID == 1 || PA_LAZY_SAMPLE_SHAPE_ID == 2,
    "PA_LAZY_SAMPLE_SHAPE_ID must select compete-first(1) or compete-first-lazy(2)"
);
constexpr bool kLazySampleLazy = PA_LAZY_SAMPLE_SHAPE_ID == 2;
constexpr bool kLazySampleSplitFinish = true;
constexpr uint32_t kLazySampleShapeId = PA_LAZY_SAMPLE_SHAPE_ID;
constexpr const char *kLazySampleShapeName =
    PA_LAZY_SAMPLE_SHAPE_ID == 1 ? "compete-first" : "compete-first-lazy";
constexpr const char *kLazySampleObservation = "split-combination-semantic";
constexpr const char *kLazySampleFinishShape = "noinline-cross-tu";
constexpr const char *kLazySampleControlFamily = "all-task-compete-first-callback";
#if defined(PA_LAZY_SAMPLE_SPLIT_FINISH)
static_assert(PA_LAZY_SAMPLE_SPLIT_FINISH == 1, "split-finish feature macro must equal one");
static_assert(kLazySampleSplitFinish, "both lazy-sample callback shapes require split finish");
#else
#error "lazy-sample callback artifacts require PA_LAZY_SAMPLE_SPLIT_FINISH"
#endif
#endif
...
#if defined(PA_LAZY_SAMPLE_SPLIT_FINISH)
constexpr uint64_t kLazySampleSplitStateCookieBase = 0x514b53504c495400ULL;
#endif
...
struct alignas(64) WorkerResult {
    ...
    uint64_t lazy_sample_split_caller_state_address;
    uint64_t lazy_sample_split_finish_state_address;
    uint64_t lazy_sample_split_finish_calls;
    uint64_t lazy_sample_split_protocol_errors;
    uint64_t lazy_sample_split_state_cookie;
    uint64_t lazy_sample_split_task_id_sum;
    uint64_t lazy_sample_split_owner_worker_id;
    uint64_t lazy_sample_split_reserved;
#endif
};
...
static_assert(offsetof(WorkerResult, lazy_sample_split_caller_state_address) == 832,
              "split WorkerResult oracle offset mismatch");
static_assert(offsetof(WorkerResult, lazy_sample_split_reserved) == 888,
