...
// The two views remain final parameter-packing operations.  query_view is a
// winner-only QK input; output_view is the produce side of a private INOUT and
// is therefore evaluated on every worker by the callback builder.
PA_DEVICE void MakeLazySampleCallbackOutputView(PaOrchestrationState &orch, uint32_t batch) {
    CopyTensorLine1(orch.output_view, orch.output);
    orch.output_view.start_offset = static_cast<uint64_t>(batch) * kPaHeads * kPaHeadDim;
    orch.output_view.ndims = 2;
    orch.output_view.manual_dep = true;
...
PA_DEVICE void MakeLazySampleCallbackQueryView(PaOrchestrationState &orch, uint32_t batch) {
    CopyTensorLine1(orch.query_view, orch.query);
    orch.query_view.start_offset = static_cast<uint64_t>(batch) * kPaHeads * kPaHeadDim;
    orch.query_view.ndims = 2;
    orch.query_view.shapes[0] = kPaHeads;
...
#if defined(PA_LAZY_SAMPLE_SHAPE_ID)
PA_DEVICE void BeginPaBatchForLazySampleCallback(PaOrchestrationState &orch, uint32_t batch) {
    // Tensor reads and arithmetic that feed more than one task stay on the
    // common path.  Descriptor construction is deferred to the task callback.
    orch.current_batch = batch;
    orch.current_sequence = ReadPaContextLength(orch, batch);
    orch.current_blocks = (orch.current_sequence + kPaBlockSize - 1) / kPaBlockSize;
}
...
#if defined(PA_LAZY_SAMPLE_SHAPE_ID)
struct LazySampleCallbackBuildCounts {
...
template <bool Lazy>
class LazySampleCallbackArgsBuilder {
public:
    PA_DEVICE LazySampleCallbackArgsBuilder(TaskArgs &args, TaskKind kind, bool won)
        : args_(args), kind_(kind), won_(won), begin_calls_(0), counts_{} {}
    ...
    PA_DEVICE const LazySampleCallbackBuildCounts &Counts() const { return counts_; }
...
private:
    ...
    LazySampleCallbackBuildCounts counts_;
};
#endif
