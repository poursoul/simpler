...
下一步不再增加 selector，也不跨 ELF 扣减。固定同一 `submit-pmu-none` ELF 做独立
Case1 系列，在每份 raw 内联合分析 SYS elapsed、PMU total、scalar busy 和 I-cache
request/miss，先判断该诊断 ELF 是否复现 P 的多数核共同伸缩，以及波动来自 PMU
cycle 数还是有效 cycle/time 比例。只有同 ELF PMU 数据能够解释波动，才据此选择
真实 scalar Submit 优化候选；否则转向同一 swimlane ELF 快慢轮或最小 per-core
Kernel 聚合。单阶段 observed 不机械扣除 empty，也不从单轮 I-cache 数直接提出生产
优化；standalone 的绝对数继续不替代真实 PA 当前结果。
