...
// cross_core 的完整 PA 实现只复用同目录下的 portable
// 执行发布协议，不从 same_core 反向 include 任一实现文件。host
// 侧在引入 pa_model.h 时尚未
// 定义设备修饰宏，因此仅在本次 include 期间提供无地址空间的窄默认值；
// CCEC kernel 会在 include 本文件前给出真实 __aicore__/__gm__ 定义。
#ifndef PA_DEVICE
#define PA_CROSS_CORE_MODEL_DEFINED_PA_DEVICE 1
#define PA_DEVICE inline
#endif
#ifndef PA_GM
#define PA_CROSS_CORE_MODEL_DEFINED_PA_GM 1
#define PA_GM
#endif
...
#endif
#if defined(PA_CROSS_CORE_MODEL_DEFINED_PA_DEVICE)
#undef PA_DEVICE
#undef PA_CROSS_CORE_MODEL_DEFINED_PA_DEVICE
#endif
...
constexpr uint32_t kAicWorkers = 8;
constexpr uint32_t kAivWorkers = 8;
...
// Host 准备沿用 ordinary 的全 worker Claim；本实验为 16 个 CPU 线程。
// kernel 类型只约束后续 Execute owner 的 engine，不约束 Host Build owner。
...
#if PTO_FDWIC_SHARED_MAP
// Host 准备保留 ordinary 的 8 组两级 Tournament 元数据格式；
// 每组先选 local owner，再竞争唯一 root。设备不执行此 Build 协议。
...
static_assert(
    kSharedAllocClaimParticipants == 16 &&
        kAicWorkers == 8 && kAivWorkers == 8,
    "scheduler-only topology requires 8 AIC and 8 AIV Scalars"
);
static_assert(
...
static_assert(
    kTraceWorkerBytes ==
        (PA_BUILD_COMPACT_GENERIC_TRACE ? 1187840U : 2236416U),
...
enum class TracePhase : int32_t {
    ...
    Dcci = 24,
    // Scheduler-only execution-side stages, not task construction.
    ExecBind = 25,
    ExecFanin = 26,
    ExecComplete = 27,
    ExecTicketBind = 28,
    // One four-slot pass. flags=completed-slot mask, auxiliary=pass index.
    ExecTokenScan = 29,
    ExecDispatch = 30,
    ExecDrainCheck = 31,
    ExecAdmission = 32,
...
// 本 benchmark 固定物理分配 kWorkers=16 个定长 record 分区，合法 header 也要求
...
// Scheduler business spans exceed 32 phases; ABI generation 53 moves one
// auxiliary bit to phase without changing the 16-byte record size.
constexpr uint32_t kCompactTracePhaseBits = 6;
constexpr uint32_t kCompactTracePhaseMask =
    (1U << kCompactTracePhaseBits) - 1U;
constexpr uint32_t kCompactTraceAuxiliaryShift = 22;
constexpr uint32_t kCompactTraceAuxiliaryBits = 10;
...
// 生成两份 task-id 表；device 侧 8 个 AIC 与 8 个 AIV 分别竞争自己的
...
#if PTO_FDWIC_TENSORMAP_RING_CAP == 128
static_assert(sizeof(SharedTensorMapSidecar) == 12429440, "shared TensorMap sidecar size changed");
static_assert(
