...
下一步不再增加 selector，也不继续比较不同 ELF。固定当前 P cache
`138ce601ea506665`，使用同一 perf-clock ELF 做至少 20 个独立 Case1，联合分析完整
墙钟、逐核 body、首核到达与最慢核尾部，专项定位独占设备上的运行波动；只有同 ELF
波动来源和可接受的 A/B 判定方法明确后，才进入真实 scalar Submit 候选优化。单阶段
observed 不机械扣除 empty，也不从单轮 I-cache 数直接提出生产优化；standalone 的
绝对数继续不替代真实 PA 当前结果。
