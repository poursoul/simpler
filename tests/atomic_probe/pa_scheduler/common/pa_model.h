...
// cursor 只由显式映射到同一 shard 的 worker 子集竞争。这是 shared
// standalone 新增的候选资格策略，不是对原 96/32/64 合同中必输调用的
// 等价删除。与固定单 owner 不同，每个 shard 内仍有多个动态候选；与
// per-task atomic 不同，后续 task 仍受同一高水位链约束。
