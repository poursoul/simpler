...
TEST_F(FdwicSubmitCapacityTest, RegisterFailureStopsBuildAndClosesFollowingClaimGate) {
    // 只留一个可分配 entry：Register 的第一个 OUTPUT_EXISTING 成功，
    // 第二个精确撞到 production kMapCap。
    worker_->map.high_water = kMapCap - 1;
    worker_->map.free_head = -1;
    const Tensor first_output = make_existing_output(0x100000);
    const Tensor second_output = make_existing_output(0x200000);
    L0TaskArgs args;
    ...
    // 第一个 region 已占用最后一个物理 entry，第二个 region 未被插入；
    // Register 失败必须在 WinnerBuild/slot 分配之前返回。
    EXPECT_EQ(worker_->map.high_water, kMapCap);
    EXPECT_EQ(worker_->map.entries[kMapCap - 1].buf_addr, first_output.buffer.addr);
