...
inline bool ExportSwimlaneRecords(
    ...
    std::fprintf(
        ...
        "\"dropped_records\":%llu}",
        kLazySampleShapeName, kLazySampleShapeId, kLazySampleObservation,
        kLazySampleFinishShape, kLazySampleControlFamily,
...
inline Metrics Validate(
    ...
    bool lazy_sample_split_runtime_oracle_ok = true;
    const uint64_t expected_lazy_sample_split_task_id_sum =
    ...
    for (uint32_t index = 0; index < kWorkers; ++index) {
        ...
        lazy_sample_split_runtime_oracle_ok &= result.worker_id == index;
        lazy_sample_split_runtime_oracle_ok &= result.lazy_sample_split_caller_state_address != 0;
        lazy_sample_split_runtime_oracle_ok &=
            result.lazy_sample_split_finish_state_address == result.lazy_sample_split_caller_state_address;
        lazy_sample_split_runtime_oracle_ok &= result.lazy_sample_split_finish_calls == task_count;
        lazy_sample_split_runtime_oracle_ok &= result.lazy_sample_split_protocol_errors == 0;
        lazy_sample_split_runtime_oracle_ok &=
            result.lazy_sample_split_state_cookie ==
                (kLazySampleSplitStateCookieBase ^ static_cast<uint64_t>(index) ^
                 (static_cast<uint64_t>(static_cast<uint32_t>(expected_role)) << 32U));
        lazy_sample_split_runtime_oracle_ok &=
            result.lazy_sample_split_task_id_sum == expected_lazy_sample_split_task_id_sum;
        lazy_sample_split_runtime_oracle_ok &= result.lazy_sample_split_owner_worker_id == index;
        lazy_sample_split_runtime_oracle_ok &= result.lazy_sample_split_reserved == 0;
#endif
#if defined(PA_LAZY_SAMPLE_SHAPE_ID)
        ...
        const uint64_t expected_worker_views = kLazySampleLazy
            ? static_cast<uint64_t>(batches) + qk_wins
            : static_cast<uint64_t>(batches) * 2;
        const uint64_t expected_worker_tensors = kLazySampleLazy
            ? static_cast<uint64_t>(batches) * 12 +
                  qk_wins * 3 + sf_wins + pv_wins * 3 + up_wins * 3
            : static_cast<uint64_t>(batches) * 22;
        const uint64_t expected_worker_scalars = kLazySampleLazy
            ? qk_wins * 2 + sf_wins * 3 + pv_wins * 2 + up_wins * 2
        ...
        // Alloc constructs once; QK/SF/PV/UP each reset once inside their callback.
    ...
    Expect(
        lazy_sample_split_runtime_oracle_ok,
        "split caller/finish share one role-specific block-local state and finish every task once",
        &metrics
    );
#endif
    ...
    std::printf(
        "[LAZY_SAMPLE_FRONTEND] shape=%s views=%llu/%llu tensor_args=%llu/%llu "
        "scalar_args=%llu/%llu resets=%llu/%llu\n",
        kLazySampleShapeName,
...
#if defined(PA_LAZY_SAMPLE_SHAPE_ID)
    std::printf(
        "lazy_sample_shape=%s lazy_sample_shape_id=%u observation=%s finish=%s "
        "control_family=%s\n",
        kLazySampleShapeName, kLazySampleShapeId, kLazySampleObservation,
        kLazySampleFinishShape, kLazySampleControlFamily
    );
